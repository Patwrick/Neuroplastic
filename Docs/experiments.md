...
[Back to Neuroplastic](../readme.md)

This guide contains the detailed commands for the current mechanism experiments and environment tooling. Run all commands from the repository root with your Python environment activated. Python 3.11 or 3.12 is recommended for the current dependency constraints.

The neural cue benchmark runs independently of Minecraft. The graph-core toy experiment is an early implementation scaffold; its limitations are documented in the [mathematical and feasibility audit](csgn_math_feasibility_audit_v0_3.md). MineStudio provides an optional Minecraft environment for recording and replaying agent interactions.

These experiments exercise individual components. They do not yet validate the complete [CSGN v0.3 architecture](csgn_paper_v0_3.md).
...
Compare within-episode reward for the two modes, especially when `change_every=0`. An adaptation advantage is a hypothesis to test across seeds, rather than an assumed result.
...
The slot modes test structural adaptation in a small cue-association system: a fixed number of slots is reassigned using a utility score. Compare uniform and Zipf cue sampling to measure how cue frequency affects adaptation under the same slot budget.
